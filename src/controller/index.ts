...
export default function chooseEndpoint(req: ReqType, res: ResType, users: User[]) {
  try {
    const { pathname: parsedPathname } = parse(req.url || '', true);

    const pathname = parsedPathname || '';
